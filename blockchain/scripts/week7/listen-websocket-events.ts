  import { ethers, WebSocketProvider, EventLog } from "ethers";
  ...
  async function listenWebSocketEvents() {
    console.log("=== WebSocket Real-time Event Listener ===\n");

    // Connect to network to get the RPC URL
    const connection = await network.connect("sepolia");

    // Get the HTTP RPC URL from the connection
    const httpUrl = connection.config.url;

    console.log("📋 Original HTTP URL:", httpUrl.substring(0, 50) + "...");

    // Convert HTTP to WebSocket
    const wsUrl = httpUrl.replace("https://", "wss://");

    console.log("📋 Converted WebSocket URL:", wsUrl.substring(0, 50) + "...");
    console.log("");
    ...
    console.log("Network: Sepolia (WebSocket)");
  ...
  `https://sepolia.etherscan.io/tx/${event.transactionHash}`);
    ...
    setInterval(() => {
      ...
      console.log(`💓 WebSocket active... ${now}`);
    }, 30000);
  ...
  listenWebSocketEvents().catch((error) => {
